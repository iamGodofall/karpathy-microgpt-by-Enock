...
import unittest
from microgpt.model import Value, GPT, softmax, rmsnorm, layernorm
...
class TestGPT(unittest.TestCase):
    ...
    def test_generation(self):
        tokens = self.model.generate(0, max_length=5, temperature=1.0)  # start token
...
class TestMultiLayer(unittest.TestCase):
    ...
    def test_two_layer_model(self):
        model = GPT(vocab_size=10, block_size=8, n_layer=2, n_embd=16, n_head=4)
...
class TestDropout(unittest.TestCase):
    ...
    def test_dropout_training(self):
        from microgpt.model import Dropout
