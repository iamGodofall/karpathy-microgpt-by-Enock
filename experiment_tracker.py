...
class ExperimentTracker:
    ...
    def get_best_experiment(self, metric: str = 'loss', mode: str = 'min') -> Optional[Experiment]:
        ...
        if mode == 'min':
            return min(completed, key=lambda e: e.metrics.get(metric, [{}])[-1].get('value', float('inf')))
        else:
            return max(completed, key=lambda e: e.metrics.get(metric, [{}])[-1].get('value', float('-inf')))
