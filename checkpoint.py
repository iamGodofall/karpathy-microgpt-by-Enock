...
class CheckpointManager:
    ...
    def save_json(self, state_dict: Dict[str, List], config: Any, 
                  step: int, loss: float, filename: Optional[str] = None):
        """Save checkpoint in JSON format (human-readable, larger)."""
        if filename is None:
            filename = f"checkpoint_step_{step}.json"
        
        # Convert Value objects to raw data
        serializable_state = {}
        for key, matrix in state_dict.items():
            serializable_state[key] = [[v.data for v in row] for row in matrix]
    ...
    def save_pickle(self, state_dict: Dict[str, List], config: Any,
                    step: int, loss: float, filename: Optional[str] = None):
    ...
    def save_best(self, state_dict: Dict[str, List], config: Any,
                  step: int, loss: float, format: str = 'pkl'):
