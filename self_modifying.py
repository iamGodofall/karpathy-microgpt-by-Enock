...
class SelfModifyingSystem:
    ...
    def get_evolution_report(self) -> str:
        ...
        for mod in self.modification_history[-5:]:
            gene = mod['gene']
            lines.append(f"- Gen {mod['generation']}: {gene.modification_type} {gene.target_function}")
...
class RecursiveImprover:
    ...
    def evolve_hierarchy(self):
        ...
        self.meta_system.evolve(
            fitness_func=lambda s: sum(
                len(sub.genes) for sub in self.subsystems
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print(f"  Gen {i+1}: {len(system.genes)} modifications, "
