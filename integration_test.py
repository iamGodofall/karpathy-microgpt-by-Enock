...
def test_core_pipeline():
    ...
    for i in range(5):
        tokens = [tokenizer.bos_token] + [tokenizer.char_to_idx[c] for c in docs[i % len(docs)]] + [tokenizer.bos_token]
...
def test_compression():
    ...
    print(f"✓ Pruning: {original_params} -> {nonzero} params ({100*(1-nonzero/original_params):.1f}% sparsity)")
