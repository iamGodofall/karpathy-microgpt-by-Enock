...
class Profiler:
    ...
    def report(self):
        ...
        # Sort by total time
        sorted_results = sorted(self.results.values(), key=lambda x: -x.total_time)
...
class MemoryTracker:
    ...
    def sample(self, label: str = ""):
        ...
        self.samples.append({"time": time.time(), "label": label, "memory_mb": memory_mb})
...
class ModelAnalyzer:
    ...
    def _estimate_memory(self) -> Dict:
        ...
        activation_memory = batch_size * seq_len * hidden_dim * n_layers * 4 / (1024**2)

        # KV cache
        kv_memory = 2 * batch_size * seq_len * hidden_dim * n_layers * 4 / (1024**2)
...
class SpeedBenchmark:
    ...
    def benchmark_generation(
        self, prompt_length: int = 10, generation_length: int = 100, num_runs: int = 10
    ) -> Dict:
        ...
        for _ in range(num_runs):
            # Random prompt
            prompt = [random.randint(0, self.model.vocab_size - 1) for _ in range(prompt_length)]

            start = time.time()

            # Generate
            self.model.generate(prompt[-1], max_length=generation_length, temperature=1.0)
    ...
    def benchmark_training_step(self, seq_length: int = 128, num_runs: int = 10) -> Dict:
        ...
        for _ in range(num_runs):
            # Random sequence
            tokens = [random.randint(0, self.model.vocab_size - 1) for _ in range(seq_length)]
