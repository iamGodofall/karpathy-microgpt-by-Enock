...
class LoRA:
    ...
    def __init__(self, in_dim: int, out_dim: int, rank: int = 8, alpha: float = 16.0):
        ...
        # Low-rank matrices
        self.lora_A = [[random.gauss(0, 0.01) for _ in range(in_dim)] for _ in range(rank)]
        self.lora_B = [[0.0 for _ in range(rank)] for _ in range(out_dim)]  # Initialize B to zero
    ...
    def forward(self, x: List[Value], base_output: List[Value]) -> List[Value]:
        ...
        # A @ x
        Ax = [
            sum(self.lora_A[i][j] * x[j].data for j in range(self.in_dim)) for i in range(self.rank)
        ]

        # B @ (A @ x)
        BAx = [
            sum(self.lora_B[i][j] * Ax[j] for j in range(self.rank)) for i in range(self.out_dim)
        ]

        # Add to base output with scaling
        return [Value(bo.data + self.scaling * ba) for bo, ba in zip(base_output, BAx)]
    ...
    def merge_weights(self, base_weight: List[List[Value]]) -> List[List[Value]]:
        ...
        BA = [
            [
                sum(self.lora_B[i][k] * self.lora_A[k][j] for k in range(self.rank))
        ...
        # Merge
        merged = [
            [Value(w.data + self.scaling * ba) for w, ba in zip(row_w, row_ba)]
...
class QLoRA:
    ...
    def __init__(self, base_model: GPT, rank: int = 64, bits: int = 4, group_size: int = 64):
        ...
        for name, matrix in self.base_model.state_dict.items():
            ...
            for g in range(0, len(matrix), self.group_size):
                ...
                quantized[(name, g)] = {
                    "min": min_val,
                    "scale": scale,
                    "values": [[int((v.data - min_val) / scale) for v in row] for row in group],
                }
    ...
    def forward(self, token_id: int, pos: int, keys: List[List], values: List[List]) -> List[Value]:
        """
...
class DoRA:
    ...
    def forward(self, x: List[Value], base_weight: List[List[Value]]) -> List[Value]:
        ...
        # Normalize direction
        norms = [sum(d[i].data ** 2 for i in range(self.in_dim)) ** 0.5 for d in direction]
        normalized = [
            [d[i].data / (n + 1e-8) for i in range(self.in_dim)] for d, n in zip(direction, norms)
        ]

        # Scale by magnitude
        scaled = [
            [self.magnitude[i] * normalized[i][j] for j in range(self.in_dim)]
            for i in range(self.out_dim)
        ]

        # Apply to input
        output = [
            sum(scaled[i][j] * x[j].data for j in range(self.in_dim)) for i in range(self.out_dim)
        ]
...
class ReLoRA:
    ...
    def __init__(self, model: GPT, rank: int = 8, restart_interval: int = 1000):
        self.model = model
        self.rank = rank
...
class GaLore:
    ...
    def _update_projection(self, name: str, grad: List[List[float]]):
        ...
        self.U[name] = [[random.gauss(0, 1) for _ in range(self.rank)] for _ in range(m)]
        self.V[name] = [[random.gauss(0, 1) for _ in range(n)] for _ in range(self.rank)]
...
class LongLoRA:
    ...
    def __init__(self, model: GPT, rank: int = 8, context_extension: int = 8):
        self.model = model
        self.rank = rank
