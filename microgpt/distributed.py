...
class DataParallelTrainer:
    ...
    def train_step(
        self, batch_docs: List[str], tokenizer, step: int, optimizer_step_fn: Callable
    ) -> float:
        """
...
class PipelineParallelTrainer:
    ...
    def __init__(
        self, model: GPT, config: DistributedConfig, layers_per_device: Optional[List[int]] = None
    ):
        self.model = model
        self.config = config
        ...
        self.local_layers = [i for i, d in enumerate(self.layer_assignments) if d == config.rank]
...
class FederatedLearning:
    ...
    def client_update(
        self, client_id: int, local_data: List[str], tokenizer, num_steps: int = 10
...
class AsyncTraining:
    ...
    def apply_gradient(self, gradients: Dict[str, List[List[float]]], gradient_step: int):
        """
...
def simulate_distributed_training(model: GPT, num_workers: int = 4, num_steps: int = 100):
    """
    ...
    configs = [
        DistributedConfig(world_size=num_workers, rank=i, local_rank=i) for i in range(num_workers)
    ]
