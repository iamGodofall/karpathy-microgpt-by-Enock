...
class ParticleSwarmOptimizer:
    ...
    def __init__(
        self, n_particles: int = 30, dim: int = 10, bounds: List[Tuple[float, float]] = None
    ):
...
class Flock:
    ...
    def __init__(self, n_boids: int = 100, width: float = 800, height: float = 600):
        self.boids = [
            Boid(random.uniform(0, width), random.uniform(0, height)) for _ in range(n_boids)
        ]
        self.width = width
        self.height = height
...
class CollectiveIntelligence:
    ...
    def initialize(self):
        ...
        self.agents = [
            {"belief": random.random(), "confidence": random.random(), "influence": random.random()}
