...
class ModelComparator:
    ...
    def compare(
        self, model_a: Any, model_b: Any, name_a: str, name_b: str, eval_funcs: Dict[str, Callable]
    ) -> ModelComparison:
    ...
    def compare_configs(
        self, configs: List[Any], names: List[str], train_func: Callable, eval_func: Callable
    ) -> Dict[str, Any]:
        ...
        for config, name in zip(configs, names):
            ...
            results.append({"name": name, "config": config, "score": score})

        # Rank by score
        ranked = sorted(results, key=lambda x: x["score"])

        return {"rankings": ranked, "best": ranked[0], "worst": ranked[-1], "comparison": results}

    def ab_test(
        self, model_a: Any, model_b: Any, test_cases: List[Dict[str, Any]], judge_func: Callable
    ) -> Dict[str, Any]:
    ...
    def generate_report(self, output_path: str = "comparison_report.md"):
        ...
        for comp in self.results:
            ...
            for metric, values in comp.metrics.items():
                ...
                lines.append(
                    f"  - Difference: {values['difference']:.4f} ({values['relative']*100:.1f}%)"
                )
...
class PerformanceComparator:
    ...
    def compare_architectures(
        architectures: List[Any], names: List[str], input_data: List
    ) -> Dict[str, Any]:
