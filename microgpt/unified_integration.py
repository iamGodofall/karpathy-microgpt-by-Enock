...
class UnifiedAI:
    ...
    def chat(
        ...
        with self.lock:
            ...
            # Generate response
            if use_reasoning:
                response, reasoning_info = self._reasoning_generate(message, session, tool_results)
            else:
    ...
    def _reasoning_generate(
        self, message: str, session: Session, tool_results: List[Dict]
    ) -> Tuple[str, Dict]:
    ...
    def train_on_session(self, session_id: str, num_steps: int = 10):
        ...
        for i in range(0, len(session.messages) - 1, 2):
            if session.messages[i].role == "user":
                ...
                target_text = (
                    session.messages[i + 1].content if i + 1 < len(session.messages) else ""
                )
...
def demo_unified_system():
    ...
    print("-" * 70)
    result = ai.chat(
        "Explain step by step how to solve a complex optimization problem", use_reasoning=True
    )
    print(f"Response: {result['response']}")
    print(f"Used reasoning: {result['used_reasoning']}")
    ...
    print("\n3. Tool Use")
    print("-" * 70)
    result = ai.chat("Calculate the sum of squares from 1 to 10", tools=["calculator"])
    print(f"Response: {result['response']}")
