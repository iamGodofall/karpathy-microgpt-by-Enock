...
class MicroGPTWithOpenClaw:
    ...
    def _init_params(self):
        ...
        # Embeddings
        self.params["wte"] = [
            [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(vocab_size)
        ]
        self.params["wpe"] = [
            [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(block_size)
        ]

        # Transformer layers
        for i in range(n_layer):
            # Attention
            self.params[f"attn_{i}_wq"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(n_embd)
            ]
            self.params[f"attn_{i}_wk"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(n_embd)
            ]
            self.params[f"attn_{i}_wv"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(n_embd)
            ]
            self.params[f"attn_{i}_wo"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(n_embd)
            ]

            # MLP (SwiGLU)
            hidden_dim = 4 * n_embd
            self.params[f"mlp_{i}_w1"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(hidden_dim)] for _ in range(n_embd)
            ]
            self.params[f"mlp_{i}_w2"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(n_embd)] for _ in range(hidden_dim)
            ]
            self.params[f"mlp_{i}_w3"] = [
                [Value(random.gauss(0, 0.02)) for _ in range(hidden_dim)] for _ in range(n_embd)
            ]
        ...
        self.params["lm_head"] = [
            [Value(random.gauss(0, 0.02)) for _ in range(vocab_size)] for _ in range(n_embd)
        ]
    ...
    def gpt(self, token_id: int, pos_id: int, keys_cache: List, values_cache: List) -> List[Value]:
        ...
        for layer in range(n_layer):
            ...
            mlp_hidden = swiglu(
                ln2_out, self.params[f"mlp_{layer}_w1"], self.params[f"mlp_{layer}_w3"]
            )
    ...
    def train_step(self, doc: str) -> float:
        ...
        # Tokenize
        tokens = (
            [self.BOS] + [self.uchars.index(ch) for ch in doc if ch in self.uchars] + [self.BOS]
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "model": {
                ...
                "vocab_size": self.vocab_size,
                "num_parameters": (
                    sum(len(row) for matrix in self.params.values() for row in matrix)
                    if self.params
                    else 0
                ),
            },
