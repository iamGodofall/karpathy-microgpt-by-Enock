...
class QuantizedTensor:
    ...
    def __init__(
        self, quantized_data: List[List[int]], scale: float, zero_point: int = 0, bits: int = 8
    ):
    ...
    def dequantize(self) -> List[List[float]]:
        ...
        return [[(q - self.zero_point) * self.scale for q in row] for row in self.data]
...
class Quantizer:
    ...
    def quantize_tensor(self, tensor: List[List[Value]]) -> QuantizedTensor:
        ...
        # Extract float values (handle both Value objects and raw floats)
        float_data = []
        for row in tensor:
            float_row = []
            for v in row:
                if hasattr(v, "data"):
                    float_row.append(v.data)
                else:
        ...
        if self.config.per_channel:
            ...
            quantized = [
                [self._quantize_value(v, scale, zero_point) for v in row] for row in float_data
            ]
    ...
    def load_quantized(cls, path: str) -> Dict[str, QuantizedTensor]:
        ...
        for name, qt_data in data.items():
            result[name] = QuantizedTensor(
                qt_data["data"], qt_data["scale"], qt_data["zero_point"], qt_data["bits"]
            )
...
class QuantizedGPT:
    ...
    def __init__(
        self,
        quantized_state: Dict[str, QuantizedTensor],
