    ...
    return jsonify(
        {
            "name": "microgpt API",
            "version": "1.0.0",
            "endpoints": ["/generate", "/chat", "/models", "/health"],
        }
    )
...
def list_models():
    """List loaded models."""
    return jsonify({"models": list(models.keys()), "active": "default" if active_model else None})
        ...
        return jsonify(
            {
                "prompt": prompt,
                "generations": results,
                "parameters": {"temperature": temperature, "top_k": top_k, "top_p": top_p},
            }
        )
...
def chat():
    ...
    try:
        ...
        generated = active_model.generate(
            tokens[-1], max_length=max_tokens, temperature=temperature
        )
...
def embeddings():
    ...
    try:
        ...
        embedding = [logit.data for logit in logits]
