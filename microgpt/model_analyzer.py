...
class ModelAnalyzer:
    ...
    def analyze(self) -> ModelAnalysis:
        ...
        recommendations = self._generate_recommendations(total_params, n_layer, n_embd, block_size)
    ...
    def _layer_params(self, n_layer: int, n_embd: int, n_head: int) -> Dict[str, int]:
        ...
        return {"attention": attn_params, "mlp": mlp_params, "total": attn_params + mlp_params}
    ...
    def _generate_recommendations(
        self, total_params: int, n_layer: int, n_embd: int, block_size: int
    ) -> List[str]:
        ...
        if total_params < 10000:
            recommendations.append(
                "Model is very small. Consider increasing n_embd or n_layer for better performance."
            )
        elif total_params > 10000000:
            recommendations.append(
                "Model is large. Training will be slow in pure Python. Consider using PyTorch for production."
            )

        # Architecture recommendations
        if n_layer < 2:
            recommendations.append(
                "Single layer may have limited capacity. Try n_layer=2-4 for better results."
            )

        if n_embd < 64:
            recommendations.append(
                "Small embedding dimension. Consider n_embd >= 64 for better representations."
            )

        if block_size < 32:
            recommendations.append(
                "Short context window. Consider block_size=64-128 for longer dependencies."
            )
    ...
    def compare_configs(self, configs: List[Any]) -> Dict[str, Any]:
        ...
        comparison["summary"]["fastest"] = min(
            analyses, key=lambda x: x.flops_per_token
        ).flops_per_token
...
if __name__ == "__main__":
    ...
    for cfg in comparison["configs"]:
        print(
            f"Config {cfg['index']}: {cfg['params']:,} params, "
