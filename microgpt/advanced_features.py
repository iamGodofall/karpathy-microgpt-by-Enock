...
class BeamSearchDecoder:
    ...
    def decode(
        self, model: GPT, start_token: int, temperature: float = 1.0
        ...
        for _ in range(self.max_length):
            ...
            for seq, score, keys, values in beams:
                ...
                top_indices = sorted(
                    range(len(probs_data)), key=lambda i: probs_data[i], reverse=True
                )[:top_k]
    ...
    def __call__(self, logits: List[Value], generated_tokens: List[int]) -> List[Value]:
        ...
        for i, logit in enumerate(logits):
            if i in token_counts:
                ...
                logit_val = logit.data if hasattr(logit, "data") else logit
                penalized.append(Value(logit_val / penalty_factor))
            else:
...
class ContrastiveSearchDecoder:
    ...
    def decode(self, model: GPT, start_token: int, max_length: int = 50) -> List[int]:
        """
...
class MirostatSampling:
    ...
    def __init__(self, target_perplexity: float = 8.0, tau: float = 3.0, max_surprise: float = 5.0):
    ...
    def sample(self, logits: List[Value]) -> int:
        ...
        valid_indices = [
            i for i, s in enumerate(surprises) if s <= self.current_surprise + self.max_surprise
        ]
        ...
        self.current_surprise = self.tau * observed_surprise + (1 - self.tau) * math.log(
            self.target_perplexity
        )
