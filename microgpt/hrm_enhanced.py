...
class AdaptiveHRMBlock:
    ...
    def __init__(self, hidden_size: int, num_heads: int, dropout: float = 0.0):
        ...
        # Attention weights
        self.wq = [
            [Value(random.gauss(0, 0.02)) for _ in range(hidden_size)] for _ in range(hidden_size)
        ]
        self.wk = [
            [Value(random.gauss(0, 0.02)) for _ in range(hidden_size)] for _ in range(hidden_size)
        ]
        ...
        self.w1 = [
            [Value(random.gauss(0, 0.02)) for _ in range(hidden_dim)] for _ in range(hidden_size)
        ]
        self.w2 = [
            [Value(random.gauss(0, 0.02)) for _ in range(hidden_size)] for _ in range(hidden_dim)
        ]
        self.w3 = [
            [Value(random.gauss(0, 0.02)) for _ in range(hidden_dim)] for _ in range(hidden_size)
        ]
    ...
    def forward(self, x, cache_k, cache_v, pos, use_cache=True):
        ...
        gated = [
            self.skip_gate[i] * x[i] + (1 - self.skip_gate[i]) * attn_out[i]
...
class MemoryAugmentedHRM:
    ...
    def read(self, query, k=3):
        ...
        self.access_weights[
            scores.index(max(scores.data if hasattr(max(scores), "data") else 0))
        ] += 0.1
...
class EnhancedHierarchicalReasoningModel:
    ...
    def __init__(self, config: EnhancedHRMConfig):
        ...
        # Embeddings
        self.embed_tokens = [
            [Value(random.gauss(0, 0.02)) for _ in range(config.hidden_size)]
        ...
        if config.num_tasks > 1:
            self.task_embeddings = [
                [Value(random.gauss(0, 0.02)) for _ in range(config.task_embedding_dim)]
        ...
        # Reasoning modules
        self.H_module = [
            AdaptiveHRMBlock(config.hidden_size, config.num_heads, config.dropout)
        ...
        # Output heads
        self.lm_head = [
            [Value(random.gauss(0, 0.02)) for _ in range(config.vocab_size)]
            for _ in range(config.hidden_size)
        ]

        # Double Q-heads for stable ACT
        self.q_head_1 = [
            [Value(random.gauss(0, 0.01)) for _ in range(2)] for _ in range(config.hidden_size)
        ]
        self.q_head_2 = [
            [Value(random.gauss(0, 0.01)) for _ in range(2)] for _ in range(config.hidden_size)
        ]
    ...
    def embed_input(self, tokens, task_id=0):
        ...
        for t in tokens:
            emb = [
                self.embed_tokens[t][i] * self.embed_scale for i in range(self.config.hidden_size)
            ]
            ...
            if self.task_embeddings:
                ...
                if len(task_emb) < self.config.hidden_size:
                    task_emb = task_emb + [
                        Value(0) for _ in range(self.config.hidden_size - len(task_emb))
                    ]
        ...
        for i, emb in enumerate(embeddings):
            ...
            embeddings[i] = [
                emb[j] + 0.707106781 * pos_emb[j] for j in range(self.config.hidden_size)
            ]
    ...
    def forward(self, tokens, task_id=0, training=True):
        ...
        while not carry.halted:
            ...
            h_cycles = (
                random.randint(self.config.min_cycles, self.config.max_cycles)
                if self.config.adaptive_depth
                else self.config.H_cycles
            )
            l_cycles = (
                random.randint(self.config.min_cycles, self.config.max_cycles)
                if self.config.adaptive_depth
                else self.config.L_cycles
            )
        ...
        if self.config.use_lr_schedule:
            if self.step < self.config.warmup_steps:
                ...
                progress = (self.step - self.config.warmup_steps) / (
                    10000 - self.config.warmup_steps
                )
    ...
    def get_stats(self):
        ...
        return {
            ...
            "avg_steps": (
                sum(self.cycle_history) / len(self.cycle_history) if self.cycle_history else 0
            ),
