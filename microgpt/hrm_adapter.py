...
class HRMReasoningModule:
    ...
    def forward(
        self, x: List[Value], injection: Optional[List[Value]] = None, pos: int = 0
    ) -> List[Value]:
...
class HierarchicalReasoningModel:
    ...
    def __init__(self, config: HRMConfig):
        ...
        if config.puzzle_emb_ndim > 0:
            ...
            self.puzzle_emb = [
                [Value(0) for _ in range(config.puzzle_emb_ndim)] for _ in range(1000)
            ]  # Max 1000 puzzles
        else:
    ...
    def forward_step(
        self, carry: HRMCarry, tokens: List[int], puzzle_id: Optional[int] = None
    ) -> Tuple[HRMCarry, List[Value], Tuple[Value, Value]]:
        """
    ...
    def should_halt(
        self, q_halt: Value, q_continue: Value, step: int, training: bool = True
    ) -> bool:
    ...
    def forward(
        self, tokens: List[int], puzzle_id: Optional[int] = None, training: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def compute_loss(
        self, tokens: List[int], targets: List[int], puzzle_id: Optional[int] = None
    ) -> Tuple[Value, Dict]:
        """
