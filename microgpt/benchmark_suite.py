...
class BenchmarkSuite:
    ...
    def benchmark(
        self, name: str, metric: str, unit: str, func: Callable, *args, **kwargs
    ) -> BenchmarkResult:
        ...
        try:
            ...
            benchmark = BenchmarkResult(
                name=name,
                metric=metric,
                value=func_result if isinstance(func_result, (int, float)) else duration,
                unit=unit,
                details=(
                    {"duration": duration} if not isinstance(func_result, dict) else func_result
                ),
            )
            ...
            print(f"✗ Error: {e}")
            benchmark = BenchmarkResult(
                name=name, metric=metric, value=0, unit=unit, details={"error": str(e)}
            )
            self.results.append(benchmark)
            return benchmark
    ...
    def _benchmark_generation(self):
        ...
        def bench_generate():
            gen_result = model.generate("h", tokenizer, max_length=20)
            return len(gen_result)
    ...
    def _benchmark_integrations(self):
        ...
        config = EnhancedHRMConfig(
            vocab_size=100, hidden_size=32, H_layers=1, L_layers=1, H_cycles=2, L_cycles=2
        )
        hrm = EnhancedHierarchicalReasoningModel(config)

        def bench_hrm():
            tokens = [1, 2, 3, 4, 5]
            hrm_result = hrm.forward(tokens, training=False)
            return hrm_result["steps"]
        ...
        def bench_unified():
            ai.chat("test", use_reasoning=False)
            return 1
            ...
            json.dump(
                {
                    ...
                    "summary": {"total": len(self.results), "categories": list(categories.keys())},
                },
                f,
                indent=2,
            )
