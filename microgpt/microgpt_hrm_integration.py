...
class HybridGPTWithHRM:
        ...
        if self.use_hybrid:
            ...
            for _ in range(config.n_layer):
                layer = {
                    ...
                    "w1": [
                        [Value(random.gauss(0, 0.02)) for _ in range(4 * config.hidden_size)]
                        for _ in range(config.hidden_size)
                    ],
                    "w2": [
                        [Value(random.gauss(0, 0.02)) for _ in range(config.hidden_size)]
                        for _ in range(4 * config.hidden_size)
                    ],
    ...
    def standard_transformer_forward(self, tokens: List[int], pos: int) -> List[Value]:
        ...
        # Embeddings
        x = [
            self.embed_tokens[tokens[pos]][i] * self.embed_scale + self.embed_pos[pos][i]
    ...
    def forward_hybrid(
        self, tokens: List[int], pos: int, puzzle_id: Optional[int] = None
    ) -> List[Value]:
        """
...
class HRMTrainer:
    ...
    def train(
        ...
        for step in range(num_steps):
            ...
            if step % 10 == 0:
                print(
                    f"Step {step}: loss={info['total_loss']:.4f}, "
