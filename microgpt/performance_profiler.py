...
class PerformanceProfiler:
    ...
    def _print_report(self):
        ...
        for name, result in sorted(
            self.profiles.items(), key=lambda x: x[1].total_time, reverse=True
        ):
            print(
                f"{name:<40} {result.calls:<8} {result.total_time:<10.4f} "
...
class ModelProfiler:
    ...
    def profile_forward(self, tokens: List[int]) -> Dict[str, Any]:
        ...
        if hasattr(self.model, "config"):
            ...
            layers = (
                config.n_layer if hasattr(config, "n_layer") else config.H_layers + config.L_layers
            )
