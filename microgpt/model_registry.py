...
class ModelRegistry:
    ...
    def _save_registry(self):
        ...
        registry_file = self.registry_dir / "registry.json"
        data = {
            model_id: [asdict(v) for v in versions] for model_id, versions in self.models.items()
        }
    ...
    def register_model(
        ...
        # Generate version
        version_hash = hashlib.md5(f"{model_id}_{time.time()}".encode()).hexdigest()[:8]
    ...
    def delete_version(self, model_id: str, version: str):
        ...
        if model_id in self.models:
            self.models[model_id] = [v for v in self.models[model_id] if v.version != version]
            self._save_registry()
            print(f"Deleted {model_id} version {version}")

    def search_models(
        self, tag: str = None, min_metric: float = None, metric_name: str = "loss"
    ) -> List[ModelVersion]:
...
class ModelPromoter:
    ...
    def promote(self, model_id: str, version: str, from_env: str, to_env: str):
