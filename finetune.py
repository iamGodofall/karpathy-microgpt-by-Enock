...
def freeze_layers(model: GPT, num_layers_to_freeze: int):
    ...
    for i in range(num_layers_to_freeze):
        ...
        for name in [f'layer{i}.attn_wq', f'layer{i}.attn_wk', 
...
def main():
    ...
    parser.add_argument('--data', type=str, required=True,
                       help='Fine-tuning data file or directory')
    parser.add_argument('--steps', type=int, default=1000,
                       help='Number of fine-tuning steps')
    parser.add_argument('--lr', type=float, default=0.001,
                       help='Learning rate (typically smaller than pretraining)')
    parser.add_argument('--dropout', type=float, default=0.1,
                       help='Dropout rate (can increase for fine-tuning)')
    parser.add_argument('--weight-decay', type=float, default=0.01,
                       help='Weight decay')
    parser.add_argument('--freeze-layers', type=int, default=0,
                       help='Number of layers to freeze')
    parser.add_argument('--gradual-unfreeze', action='store_true',
                       help='Use gradual unfreezing (ULMFiT)')
