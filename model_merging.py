...
class TaskArithmetic:
    ...
    @staticmethod
    def merge_models(base_model: GPT, 
                     task_models: List[GPT],
                     weights: List[float]) -> GPT:
        """
...
class TIESMerging:
    ...
    @staticmethod
    def merge(base_model: GPT,
              models: List[GPT],
              weights: Optional[List[float]] = None) -> GPT:
        """
        ...
        for name in base_model.state_dict:
            for i, row in enumerate(base_model.state_dict[name]):
                for j, v in enumerate(row):
                    merged.state_dict[name][i][j].data = (
                        v.data + merged_vector[name][i][j]
                    )
...
class ModelSoups:
    ...
    @staticmethod
    def average_models(models: List[GPT], 
                       weights: Optional[List[float]] = None) -> GPT:
        """
...
class BreadthFirstMerging:
    ...
    @staticmethod
    def merge(models: List[GPT], 
              base_model: GPT,
              breadth_factor: float = 0.3) -> GPT:
        """
...
class FisherWeightedMerging:
    ...
    def merge(base_model: GPT,
              models: List[GPT],
              data_samples: List[List[int]]) -> GPT:
        """
        Merge using Fisher-weighted averaging.
        """
        # Compute Fisher for each model
        fishers = [FisherWeightedMerging.compute_fisher(m, data_samples) 
