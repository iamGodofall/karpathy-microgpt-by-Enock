...
class AttentionVisualizer:
    ...
    def get_attention_flow(self, layer: int, head: int, 
                          target_pos: int) -> List[float]:
        """
    ...
    def export_attention_html(self, tokens: List[int], 
                              tokenizer,
                              filename: str = "attention.html"):
        """
...
class NeuronAnalyzer:
    ...
    def find_maximally_activating_tokens(self, layer: int, neuron: int,
                                          texts: List[str],
                                          tokenizer) -> List[Tuple[str, float]]:
        """
    ...
    def analyze_neuron_specialization(self, layer: int, neuron: int,
        ...
        if all_activations:
            ...
            results['responsive_tokens'] = [
                {'text': t, 'position': p, 'activation': a}
                for t, p, a in top_activating
            ]
...
class ProbingClassifier:
    ...
    def train_probe(self, name: str, 
        ...
        self.probes[name] = [[random.gauss(0, 0.01) for _ in range(input_dim)] 
        ...
        for _ in range(100):
            for rep, label in zip(representations, labels):
                # Forward
                logits = [sum(w[i] * rep[i] for i in range(input_dim)) 
                         for w in self.probes[name]]
    ...
    def evaluate_probe(self, name: str,
                       representations: List[List[float]],
        ...
        correct = 0
        for rep, label in zip(representations, labels):
            logits = [sum(w[i] * rep[i] for i in range(len(rep))) 
                     for w in self.probes[name]]
...
class SaliencyAnalyzer:
    ...
    def integrated_gradients(self, tokens: List[int], 
                            target_pos: int,
