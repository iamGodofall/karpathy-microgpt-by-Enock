...
class Trainer:
    ...
    def validate(self, val_docs: List[str], char_to_idx: dict, bos_token: int) -> float:
        ...
        for doc in val_docs:
            tokens = [bos_token] + [char_to_idx[ch] for ch in doc if ch in char_to_idx] + [bos_token]
            if len(tokens) < 2:
                continue
    ...
    def train(self, train_docs: List[str], val_docs: Optional[List[str]], 
              char_to_idx: dict, bos_token: int,
