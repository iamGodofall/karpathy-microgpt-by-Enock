...
class SpeedBenchmark:
    ...
    def benchmark_training(self, num_steps: int = 100, 
                          sequence_length: int = 16) -> BenchmarkResult:
    ...
    def benchmark_inference(self, num_samples: int = 100,
                           max_length: int = 50) -> BenchmarkResult:
...
class QualityMetrics:
    ...
    def calculate_perplexity(self, texts: List[str]) -> float:
        ...
        for text in texts:
            tokens = [self.tokenizer.bos_token] + [
                ...
                for ch in text if ch in self.tokenizer.char_to_idx
    ...
    def diversity_metrics(self, generated_texts: List[str]) -> Dict:
        ...
        # Repetition rate
        repeats = sum(1 for i in range(1, len(all_tokens)) 
    ...
    def _print_comparison(self):
        ...
        for result in self.results:
            print(f"{result.name:<30} {result.total_time:<12.2f} "
                  f"{result.samples_per_second:<12.2f} {result.memory_estimate_mb:<12.2f}")
