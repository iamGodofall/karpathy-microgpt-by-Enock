...
class ABTestManager:
    ...
    def record_result(self, test_id: str, model_used: str, 
    ...
    def analyze_test(self, test_id: str, metric: str = 'loss') -> Dict[str, Any]:
        ...
        model_a_metrics = [
            ...
            if r['model'] == test.model_a
        ]
        model_b_metrics = [
            r['metrics'].get(metric, 0) 
            for r in test.results 
            if r['model'] == test.model_b
        ]
        ...
        if stats['model_a']['mean'] < stats['model_b']['mean']:
            ...
            stats['improvement'] = (stats['model_b']['mean'] - stats['model_a']['mean']) / stats['model_b']['mean']
        else:
            stats['winner'] = test.model_b
            stats['improvement'] = (stats['model_a']['mean'] - stats['model_b']['mean']) / stats['model_a']['mean']
    ...
    def conclude_test(self, test_id: str, metric: str = 'loss') -> Optional[str]:
        ...
        test.confidence = min(stats['model_a']['count'], stats['model_b']['count']) / 100  # Simplified
