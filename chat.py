...
class ChatBot:
    ...
    def _softmax(self, logits: List) -> List:
        ...
        import math
        max_val = max(l.data if hasattr(l, 'data') else l for l in logits)
        exps = [math.exp((l.data if hasattr(l, 'data') else l) - max_val) for l in logits]
        total = sum(exps)
