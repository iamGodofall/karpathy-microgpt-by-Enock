...
def train_command(args):
    ...
    best_val_loss = trainer.train(
        train_docs, val_docs if val_docs else None,
        tokenizer.char_to_idx, tokenizer.bos_token,
        callback
    )
    ...
    final_path = checkpoint_mgr.save_pickle(
        model.state_dict, config, config.training.num_steps, 
...
def generate_command(args):
    ...
    print(f"Temperature: {config.generation.temperature}, Top-k: {config.generation.top_k}, Top-p: {config.generation.top_p}")
    print("-" * 50)
...
Examples:
  ...
  python cli.py generate --interactive
    ...
    gen_parser.add_argument('--checkpoint-dir', type=str, default='checkpoints',
    ...
    eval_parser.add_argument('--checkpoint-dir', type=str, default='checkpoints',
                            help='Checkpoint directory')
