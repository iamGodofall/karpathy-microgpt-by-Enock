...
class Value:
    ...
    def gelu(self):
        ...
        local_grad = cdf + x * 0.5 * math.sqrt(2.0 / math.pi) * (1 - tanh_val**2) * (1 + 3 * 0.044715 * x**2)
...
class GPT:
    ...
    def __init__(self, vocab_size: int, block_size: int, n_layer: int = 1,
                 ...
                 use_gelu: bool = False, use_layernorm: bool = False):
    ...
    def forward(self, token_id: int, pos_id: int, 
        ...
        for li in range(self.n_layer):
            ...
            for h in range(self.n_head):
                ...
                attn_logits = [sum(q_h[j] * k_h[t][j] for j in range(self.head_dim)) / 
                              (self.head_dim ** 0.5) for t in range(len(k_h))]
    ...
    def generate(self, token_id: int, max_length: int, temperature: float = 0.5,
                 top_k: int = 0, top_p: float = 1.0, 
                 keys: Optional[List[List]] = None, 
                 values: Optional[List[List]] = None) -> List[int]:
