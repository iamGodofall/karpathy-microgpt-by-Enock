...
# Import all modules from microgpt package
from microgpt.config import Config, ModelConfig, TrainingConfig, GenerationConfig
from microgpt.model import GPT
from microgpt.trainer import Trainer
from microgpt.data import CharTokenizer, load_data
from microgpt.checkpoint import CheckpointManager
from microgpt.logger import TrainingLogger
from microgpt.model_zoo import create_model, list_models
from microgpt.tokenizers import create_tokenizer, list_tokenizers
...
def train_command(args):
    ...
    # Create model
    print(
        f"Creating model: {config.model.n_layer} layers, "
    ...
    def callback(step, loss, lr, val_loss=None):
        from microgpt.logger import Metrics
        import math
        ...
        if step % config.training.save_interval == 0:
            path = checkpoint_manager.save_pickle(model.state_dict, config, step, loss)
            logger.log_event("checkpoint", {"path": str(path)})

    # Train
    print("\nStarting training...")
    best_loss = trainer.train(
        train_docs, val_docs, tokenizer.char_to_idx, tokenizer.bos_token, callback=callback
    )

    # Save final
    checkpoint_manager.save_best(model.state_dict, config, config.training.num_steps, best_loss)
    logger.save_summary()
...
def generate_command(args):
    ...
    print(
        f"Temperature: {gen_config.temperature}, Top-k: {gen_config.top_k}, Top-p: {gen_config.top_p}"
    )
    print("-" * 60)
...
def chat_command(args):
    ...
    # Load model
    from microgpt.chat import ChatSession
...
def zoo_command(args):
    ...
    for name, config in models.items():
        ...
        print(
            f"  {name:15} | {config['n_layer']}L/{config['n_embd']}D/{config['n_head']}H | ~{params:,} params"
        )
...
def estimate_params(config: dict) -> int:
    ...
    # Per layer
    params += l * (
        4 * d * d + 2 * 4 * d * d  # 4 linear in attention  # 2 MLP layers (4x expansion)
    )
