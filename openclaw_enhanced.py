...
class Session:
    ...
    def smart_compact(self, keep_recent: int = 4):
        ...
        important_msgs = [self.messages[i] for i in sorted(important_indices) 
                         if self.messages[i].role != "system"]
        recent_msgs = self.messages[-keep_recent:]
        
        # Summarize non-important messages
        other_msgs = [m for i, m in enumerate(self.messages[:-keep_recent])
...
class EnhancedOpenClaw:
    ...
    def _save_auth(self):
        ...
        data = {
            "profiles": {
                pid: asdict(p) for pid, p in self.auth_profiles.items()
            }
        }
    ...
    def chat(
        ...
        session.add_message(
            role="assistant",
            content=response,
            tool_calls=[{"tool": r["tool"], "result": r.get("result")} for r in tool_results if r.get("success")],
        )
    ...
    def _assess_complexity(self, prompt: str) -> ThinkLevel:
        ...
        complexity_indicators = [
            "explain", "analyze", "compare", "reason", "solve",
            "step by step", "why", "how does", "what if"
        ]
