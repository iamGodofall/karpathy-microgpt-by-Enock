...
def demo_complete_workflow():
    ...
    config = Config(
        ...
        generation=GenerationConfig(temperature=0.7, top_k=40, top_p=0.9, max_length=50),
    )
    ...
    result = ai.chat(
        "Explain the theory of relativity step by step", use_reasoning=True, max_reasoning_steps=5
    )
    print(f"Reasoning response: {result['response'][:50]}...")
    print(f"Reasoning steps used: {result.get('reasoning_info', {}).get('steps', 0)}")

    # Tool use
    result = ai.chat("Calculate 15 * 23", tools=["calculator"])
    print(f"Tool result: {result}")
    ...
    path = checkpoint_mgr.save_pickle(state_dict, config, step=50, loss=history["train_losses"][-1])
