...
class ByteLevelBPE:
        ...
        while len(self.vocab) < self.vocab_size:
            ...
            if new_token not in self.vocab:
                ...
                for text in byte_texts:
                    ...
                    i = 0
                    while i < len(text):
                        if i < len(text) - 1 and bytes([text[i]]) == best[0] and bytes([text[i+1]]) == best[1]:
    ...
    def encode(self, text: str) -> List[int]:
        ...
        for merge in self.merges:
            ...
            i = 0
            while i < len(tokens):
                if i < len(tokens) - 1 and bytes([tokens[i]]) == merge[0] and bytes([tokens[i+1]]) == merge[1]:
    ...
    def decode(self, tokens: List[int]) -> str:
        ...
        byte_string = b''.join(
            id_to_byte.get(t, b'') for t in tokens
        )
...
class TiktokenStyleTokenizer:
    ...
    def train(self, texts: List[str]):
        ...
        # Add special tokens
        special = ['<|endoftext|>', '<|fim_prefix|>', '<|fim_middle|>', '<|fim_suffix|>', '<|im_start|>', '<|im_end|>']
...
def create_tokenizer(tokenizer_type: str, vocab_size: int = 30000):
    ...
    if tokenizer_type not in tokenizers:
        raise ValueError(f"Unknown tokenizer: {tokenizer_type}. Choose from: {list(tokenizers.keys())}")
